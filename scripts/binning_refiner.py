...
for each_folder in input_bin_folder_list:
    ...
    os.mkdir('%s/%s/%s_new' % (wd, output_folder, os.path.basename(each_folder)))
    ...
    for each_bin in each_folder_bins:
        ...
        new = open('%s/%s/%s_new/%s_%s.fasta' % (wd, output_folder, os.path.basename(each_folder), os.path.basename(each_folder), bin_file_name), 'w')
        for each_contig in each_bin_content:
            each_contig_new_id = '%s%s%s%s%s' % (os.path.basename(each_folder), separator, bin_file_name, separator, each_contig.id)
            each_contig.id = each_contig_new_id
            each_contig.description = ''
            SeqIO.write(each_contig, new, 'fasta')
        new.close()
    # Combine all new bins
    os.system('cat %s/%s/%s_new/*.fasta > %s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(each_folder), wd, output_folder, os.path.basename(each_folder)))
    os.system('rm -r %s/%s/%s_new' % (wd, output_folder, os.path.basename(each_folder)))

# combine all modified bins together
sleep(1)
print('Combine all bins together')
if len(input_bin_folder_list) == 2:
    pwd_combined_folder_1_bins = '%s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(input_bin_folder_1))
    pwd_combined_folder_2_bins = '%s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(input_bin_folder_2))
...
if len(input_bin_folder_list) == 3:
    pwd_combined_folder_1_bins = '%s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(input_bin_folder_1))
    pwd_combined_folder_2_bins = '%s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(input_bin_folder_2))
    pwd_combined_folder_3_bins = '%s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(input_bin_folder_3))
...
for each_refined_bin in refined_bins:
    ...
    input_contigs_file = '%s/%s/combined_%s_bins.fa' % (wd, output_folder, os.path.basename(input_bin_folder_1))
